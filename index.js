...
const scraper = async(lang, country, keyword, start) => {
...
const scraper_handler = async (locale, keyword, start) => {
	try {
		...
		const scraper_data = await scraper(lang, country, keyword, start);
		return {
			statusCode: 200,
...
exports.handler = async (event) => {
	if(event.pathParameters && typeof event.pathParameters.locale !== "undefined" && typeof event.pathParameters.keyword !== "undefined" && typeof event.pathParameters.start !== "undefined")
		return scraper_handler(event.pathParameters.locale, event.pathParameters.keyword, event.pathParameters.start);
